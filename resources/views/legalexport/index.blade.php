        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">عرض صادر الشؤون القانونية </h3>
                </div>
                <div class="card-toolbar">

                    <!--begin::Button-->
                    <a href="https://master/CoreArchive/public/legalexport/create" class="btn btn-primary font-weight-bolder">
            ...
            <div class="card-body">
                        ...
                        <div id="collapseOne5" class="collapse pl-5" data-parent="#accordionExample5"
                            ...
                            <div class="card-body1">
                                <div class="col-lg-9 mb-6">
                                    <label>التاريخ:</label>
                                    <form action="{{ url('legalexport/search') }}" method="POST" class="form-group pr-5">
                                        @csrf
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($legalexport as $legalexports)
                            <tr>
                                <td> {{ $legalexports->title }}</td>
                                <td> {{ $legalexports->export->export_name }}</td>
                                <td>{{ $legalexports->number }}</td>
                                <td>{{ $legalexports->date }}</td>
                                <td>
                                    @if ($legalexports->files)
                                        @foreach (json_decode($legalexports->files) as $file)
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $legalexports->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/legalexport' . '/' . $legalexports->id) }}"
                                            accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button type="submit" class="btn btn-sm btn-clean btn-icon"
                                                title="Delete legalexport"
                                        ...
                                        <a href="{{ url('/legalexport/' . $legalexports->id . '/edit') }}"
