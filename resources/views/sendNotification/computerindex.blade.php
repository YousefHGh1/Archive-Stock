        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">عرض مراسلات الحاسوب </h3>
                </div>
                <div class="card-toolbar">

                    <!--begin::Button-->
                    <a href="https://master/CoreArchive/public/sendNotification/computerNot/create" class="btn btn-primary font-weight-bolder">
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($computerNot as $computerNots)
                            <tr>
                                <td> {{ $computerNots->id }}</td>
                                <td> {{ $computerNots->sender }}</td>
                                <td> {{ $computerNots->reciver }}</td>
                                <td>{{ $computerNots->description }}</td>
                                <td><a href="https://master/CoreArchive/public/filecomputerNot/{{ $computerNots->file }}"
                                        target="_blank">{{ $computerNots->file }}</a></td>

                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    class="datatable-cell">
                                    <span style="overflow: visible; position: relative; width: 110px;">

                                        {{-- <a href="#" onclick="confirmDelete('{{ $computerNots->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/sendNotification/computerNot' . '/' . $computerNots->id) }}"
                                            accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button type="submit" class="btn btn-sm btn-clean btn-icon"
                                                title="Delete computerNot"
                                        ...
                                        {{-- <a href="{{ url('/sendNotification/computerNot/' . $computerNots->id . '/edit') }}"
