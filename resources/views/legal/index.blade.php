        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">عرض وارد الشؤون القانونية </h3>
                </div>
                <div class="card-toolbar">

                    <!--begin::Button-->
                    <a href="https://master/CoreArchive/public/legal/create" class="btn btn-primary font-weight-bolder">
                        ...
                        <div id="collapseOne5" class="collapse pl-5" data-parent="#accordionExample5"
                            ...
                            <div class="card-body1">
                                <div class="col-lg-9 mb-6">
                                    <label>التاريخ:</label>
                                    <form action="{{ url('legal/search') }}" method="POST" class="form-group pr-5">
                                        @csrf
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    <thead class="thead_dark">
                        <tr>
                            ...
                            <th width="15%">{{ 'الجهة الوارد اليها' }}</th>
                            <th width="7%">{{ 'رقم الوارد' }}</th>
                    ...
                    <tbody>
                        @foreach ($legal as $legals)
                            <tr>
                                <td> {{ $legals->title }}</td>
                                <td> {{ $legals->import->import_name }}</td>
                                <td>{{ $legals->number }}</td>
                                <td>{{ $legals->date }}</td>
                                <td>
                                    @if ($legals->files)
                                        @foreach (json_decode($legals->files) as $file)
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $legals->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/legal' . '/' . $legals->id) }}"
                                        ...
                                        <a href="{{ url('/legal/' . $legals->id . '/edit') }}"
