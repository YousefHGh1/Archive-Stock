        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">عرض وارد الحاسوب </h3>
                </div>
                <div class="card-toolbar">

                    <!--begin::Button-->
                    <a href="https://master/CoreArchive/public/computer/create" class="btn btn-primary font-weight-bolder">
                        ...
                        <div id="collapseOne5" class="collapse pl-5" data-parent="#accordionExample5"
                            ...
                            <div class="card-body1">
                                <div class="col-lg-9 mb-6">
                                    <label>التاريخ:</label>
                                    <form action="{{ url('computer/search') }}" method="POST" class="form-group pr-5">
                                        @csrf
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($computer as $computers)
                            <tr>
                                <td> {{ $computers->title }}</td>
                                <td> {{ $computers->import->import_name }}</td>
                                <td>{{ $computers->number }}</td>
                                <td>{{ $computers->date }}</td>
                                <td>
                                    @if ($computers->files)
                                        @foreach (json_decode($computers->files) as $file)
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $computers->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/computer' . '/' . $computers->id) }}"
                                        ...
                                        <a href="{{ url('/computer/' . $computers->id . '/edit') }}"
