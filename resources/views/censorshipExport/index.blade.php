        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">عرض صادر الرقابة </h3>
                </div>
                <div class="card-toolbar">

                    <!--begin::Button-->
                    <a href="https://master/CoreArchive/public/censorshipExport/create" class="btn btn-primary font-weight-bolder">
                        ...
                        <div id="collapseOne5" class="collapse pl-5" data-parent="#accordionExample5"
                            ...
                            <div class="card-body1">
                                <div class="col-lg-9 mb-6">
                                    <label>التاريخ:</label>
                                    <form action="{{ url('censorshipExport/search') }}" method="POST" class="form-group pr-5">
                                        @csrf
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    <thead class="thead_dark">
                        <tr>
                            ...
                            <th width="17%">{{ 'الجهة الصادر اليها' }}</th>
                            <th width="8%">{{ 'رقم الصادر' }}</th>
                            <th width="10%">{{ 'تاريخ الصادر ' }} </th>
                            <th width="15%">{{ 'مرفق الصادر' }}</th>
                            <th width="10%">{{ 'الاجراءات ' }} </th>
                        </tr>
                    </thead>

                    <tbody>
                        @foreach ($censorshipExport as $censorshipExports)
                            <tr>
                                <td> {{ $censorshipExports->title }}</td>
                                <td> {{ $censorshipExports->export->export_name }}</td>
                                <td>{{ $censorshipExports->number }}</td>
                                <td>{{ $censorshipExports->date }}</td>
                                <td>
                                    @if ($censorshipExports->files)
                                        @foreach (json_decode($censorshipExports->files) as $file)
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $censorshipExports->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/censorshipExport' . '/' . $censorshipExports->id) }}"
                                            accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button type="submit" class="btn btn-sm btn-clean btn-icon"
                                                title="Delete censorshipExport"
                                        ...
                                        <a href="{{ url('/censorshipExport/' . $censorshipExports->id . '/edit') }}"
