                ...
                <div class="card card-custom gutter-b example example-compact">
                    <div class="card-header">
                        <div class="card-title">
                            <h3 class="card-label"> عرض بيانات الصنف : {{ $item->item_name }} <i class="mr-2"></i></h3>
                        </div>
                                    ...
                                    <select class="form-control selectpicker " data-size="7" tabindex="null"
                                        ...
                                        @foreach ($category as $categorys)
                                            <option value="{{ $categorys->id }}"
                                                {{ $item->category_id == $categorys->id ? 'selected' : '' }} disabled>
                                    ...
                                    <select class="form-control selectpicker " data-size="7" tabindex="null"
                                        ...
                                        @foreach ($unit as $units)
                                            <option value="{{ $units->id }}"
                                                {{ $item->unit_id == $units->id ? 'selected' : '' }} disabled>
                                                {{ $units->unit_name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-9">
                                <div class="input-group">
                                    <input name="low_limit" id="low_limit" type="number" min="0"
                                        class="form-control" value="{{ $item->low_limit }}" disabled>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        <div class="row">
                            ...
                            <div class="col-lg-10">
                                ...
                                <a type="submit" href="{{ route('item.create') }}" class="ml-2 btn btn-primary">عودة</a>
