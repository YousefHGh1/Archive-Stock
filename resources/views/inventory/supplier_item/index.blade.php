                ...
                <table id="example" class="table dt-responsive" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($supplier_item as $supplier_items)
                            <tr>
                                <td> {{ $supplier_items->supplier_item_num }}</td>
                                <td> {{ $supplier_items->supplier_item_name }}</td>
                                <td> {{ $supplier_items->address }}</td>
                                <td>{{ $supplier_items->phone }}</td>

                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    class="datatable-cell">
                                    <span style="overflow: visible; position: relative; width: 110px;">

                                        {{-- <a href="#" onclick="confirmDelete('{{ $supplier_items->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <a href="{{ url('/inventory/supplier_item/' . $supplier_items->id) }}"
                                            title="View supplier_items"><button class="btn btn-sm btn-clean btn-icon">
                                        ...
                                        <form method="POST"
                                            action="{{ url('/inventory/supplier_item' . '/' . $supplier_items->id) }}"
                                            accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button type="submit" class="btn btn-sm btn-clean btn-icon"
                                                title="Delete supplier_item"
                                        ...
                                        <a href="{{ url('/inventory/supplier_item/' . $supplier_items->id . '/edit') }}"
