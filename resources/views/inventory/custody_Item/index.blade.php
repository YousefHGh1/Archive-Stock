        ...
        <div class="card card-custom">
            <div class="flex-wrap pb-0 border-0 card-header">
                <div class="card-title">
                    <h3 class="card-label">عرض أصناف العهد </h3>
                </div>
                <div class="pb-5 card-toolbar">
                    <!--begin::Button-->
                    <a href="{{ url('/inventory/custody_Item/create') }}" class="btn btn-primary font-weight-bolder">
                ...
                <table id="example" class="table dt-responsive" style="width:100%">
                    <thead class="thead_dark">
                        <tr>
                            ...
                            <th>{{ 'رقم صنف العهدة' }} </th>
                            <th>{{ 'اسم صنف العهدة' }}</th>
                    ...
                    <tbody>
                        @foreach ($custody_Item as $custody_Items)
                            <tr>

                                <td>{{ $custody_Items->id }}</td>
                                <td>{{ $custody_Items->name }}</td>
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $custody_Items->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        {{-- <a href="{{ url('/inventory/custody_Item/' . $custody_Items->id) }}" title="View custody_Items"><button
                                        ...
                                        {{-- <a href="{{ route('transactions', ['custody_ItemId' => $custody_Items->id]) }}"
                                            title="View custody_Items">
                                        ...
                                        <form method="POST"
                                            action="{{ url('/inventory/custody_Item' . '/' . $custody_Items->id) }}"
                                            accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button type="submit" class="btn btn-sm btn-clean btn-icon"
                                                title="Delete custody_Item"
                                        ...
                                        <a href="{{ url('/inventory/custody_Item/' . $custody_Items->id . '/edit') }}"
