        ...
        <div class="card card-custom">
            ...
            <div class="card-body">
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($jibayaexport as $jibayaexports)
                            <tr>
                                <td> {{ $jibayaexports->title }}</td>
                                <td> {{ $jibayaexports->export->export_name }}</td>
                                <td>{{ $jibayaexports->number }}</td>
                                <td>{{ $jibayaexports->date }}</td>
                                <td>
                                    @if ($jibayaexports->files)
                                        @foreach (json_decode($jibayaexports->files) as $file)
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $jibayaexports->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/jibayaexport' . '/' . $jibayaexports->id) }}"
                                            accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button type="submit" class="btn btn-sm btn-clean btn-icon"
                                                title="Delete jibayaexport"
                                        ...
                                        <a href="{{ url('/jibayaexport/' . $jibayaexports->id . '/edit') }}"
