        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">عرض وارد الجباية </h3>
                </div>
                <div class="card-toolbar">

                    <!--begin::Button-->
                    <a href="https://master/CoreArchive/public/jibaya/create" class="btn btn-primary font-weight-bolder">
            ...
            <div class="card-body">
                        ...
                        <div id="collapseOne5" class="collapse pl-5" data-parent="#accordionExample5"
                            ...
                            <div class="card-body1">
                                <div class="col-lg-9 mb-6">
                                    <label>التاريخ:</label>
                                    <form action="{{ url('jibaya/search') }}" method="POST" class="form-group pr-5">
                                        @csrf
                ...
                <table id="example" class="table table-striped dt-responsive" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($jibaya as $jibayas)
                            <tr>
                                <td> {{ $jibayas->title }}</td>
                                <td> {{ $jibayas->import->import_name }}</td>
                                <td>{{ $jibayas->number }}</td>
                                <td>{{ $jibayas->date }}</td>
                                <td>
                                    @if ($jibayas->files)
                                        @foreach (json_decode($jibayas->files) as $file)
                                ...
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="#" onclick="confirmDelete('{{ $jibayas->id }}', this)"
                                        class="btn btn-sm btn-clean btn-icon" title="Delete"><i
                                            class="la la-trash"></i></a> --}}
                                        <form method="POST" action="{{ url('/jibaya' . '/' . $jibayas->id) }}"
                                        ...
                                        <a href="{{ url('/jibaya/' . $jibayas->id . '/edit') }}"
